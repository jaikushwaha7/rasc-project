"""
RASC Streamlit Demo - Enhanced Version with FIXED LABELS
...
with st.sidebar:
    ...
    label_position = st.radio(
        "Label Position",
        ["Above box", "Inside box (top-left)", "Below box"],
        index=0
    )
...
if st.button("🚀 Run Analysis", type="primary", use_container_width=True):
    ...
    try:
        ...
        # FIXED: Better font loading with fallback
        font = None
        font_size = 20
        ...
        if font is None:
            try:
                # Try loading default PIL font with size
        ...
        for idx, (cls, box) in enumerate(zip(results.boxes.cls, results.boxes.xywh)):
            ...
            # FIXED: Better bounding box drawing
            x, y, w, h = box.tolist()
            ...
            if show_object_ids:
                ...
                # FIXED: Calculate label position and background
                if font:
                ...
                # Determine label position based on user choice
                if label_position == "Above box":
                    text_x = x0
                    text_y = max(5, y0 - text_height - 5)  # Above box, with margin
                elif label_position == "Below box":
                    text_x = x0
                    text_y = min(image.height - text_height - 5, y1 + 5)  # Below box
                else:  # Inside box (top-left)
                    text_x = x0 + 5
                    text_y = y0 + 5
                ...
                draw.rectangle(bg_coords, fill=bbox_color)
